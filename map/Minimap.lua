...
function Minimap:update(x,y)
	...
	if self.moving then
		local viewX = math.clamp(love.mouse.getX() - self.x - (self.winWidth/2), view.xmin, view.xmax)
		local viewY = math.clamp(love.mouse.getY() - self.y - (self.winHeight/2),  view.ymin, view.ymax)
